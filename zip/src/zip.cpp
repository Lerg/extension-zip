...
struct zip_t *zip_open_mem(const void *buffer, size_t buffer_size) {
	struct zip_t *zip = NULL;
	...
	if (!zip)
		goto cleanup;
	if (!mz_zip_reader_init_mem(&(zip->archive), buffer, buffer_size, MZ_ZIP_FLAG_DO_NOT_SORT_CENTRAL_DIRECTORY)) {
		// An archive file does not exist or cannot initialize
		// zip_archive reader
		goto cleanup;
	}
